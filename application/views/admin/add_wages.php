...
<!-- Mirrored from thememinister.com/health/dep-list.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 02 Jul 2024 05:55:49 GMT -->
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                ...
                <div class="header-title">
                    ...
                    <h1>Wages</h1>
                    <small>Wages table</small>
                    <ol class="breadcrumb hidden-xs">
                        <li><a href="index-2.html"><i class="pe-7s-home"></i> Home</a></li>
                        <li class="active">Add_wages</li>
                    </ol>
                </div>
            </section>
            <!-- Main content -->
                    ...
                    <div class="col-sm-12">
                        <div class="panel panel-bd lobidrag">
                            <div class="panel-heading">
                                ...
                                <div class="btn-group">
                                    <a class="btn btn-primary" data-toggle="modal" data-target="#exampleModal"> <i class="fa fa-wpforms" aria-hidden="true"></i>
                                        Wages form </a>
                                </div>
                            </div>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h2><?php echo isset($wage) ? 'Edit Wages' : 'Add Wages'; ?></h2>

                                            <!-- <h3 class="modal-title" id="exampleModalLabel">Accounts form</h3> -->
                                        ...
                                        <div class="modal-body">
                                            <?php
                                            $current_date = isset($wage['date']) ? $wage['date'] : date('Y-m-d');
                                            ?>
                                            <form action="<?= isset($wage) ? base_url('WagesController/update_Wages/' . $wage['id']) : base_url('WagesController/insert_wages') ?>" method="POST">

                                                <div class="form-group">
                                                    <label>Select Employee</label>
                                                    <select class="form-control" id="department" name="emp_id">
                                                        <option disabled selected value="">Select Employee</option>
                                                        <?php foreach ($employees as $employee) : ?>
                                                            <option value="<?= $employee->emp_id ?>" <?= set_select('emp_id', $employee->emp_id, isset($wage['emp_id']) && $wage['emp_id'] == $employee->emp_id) ?>>
                                                                <?= $employee->emp_name ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" id="department" name="l_type">
                                                        <option disabled selected value="">Select Type</option>
                                                        <option value="Advance" <?= set_select('loan_type', 'Advance', isset($wage['loan_type']) && $wage['loan_type'] == 'Advance') ?>>Advance</option>
                                                        <option value="Loan" <?= set_select('loan_type', 'Loan', isset($wage['loan_type']) && $wage['loan_type'] == 'Loan') ?>>Loan</option>
                                                    </select>
                                                    <div class="text-danger"><?php echo form_error('loan_type'); ?></div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Select Type</label>
                                                    <select class="form-control" id="department" name="a_type">
                                                        <option disabled selected value="">Select Type</option>
                                                        <option value="Add" <?= set_select('a_type', 'Add', isset($wage['a_r_type']) && $wage['a_r_type'] == 'Add') ?>>Add</option>
                                                        <option value="Receive" <?= set_select('a_type', 'Receive', isset($wage['a_r_type']) && $wage['a_r_type'] == 'Receive') ?>>Receive</option>
                                                    </select>
                                                    <div class="text-danger"><?php echo form_error('a_type'); ?></div>
                                                </div>

                                                <div class="form-group">
                                                    <label>Amount</label>
                                                    <input type="text" step="0.01" class="form-control" placeholder="Enter Amount" name="amount" value="<?php echo isset($wage['amount']) ? $wage['amount'] : ''; ?>">
                                                    <div class="text-danger"><?php echo form_error('amount'); ?></div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description</label>
                                                    <textarea class="form-control" rows="3" placeholder="Enter Description" name="description"><?php echo isset($wage['description']) ? $wage['description'] : ''; ?></textarea>
                                                    <div class="text-danger"><?php echo form_error('description'); ?></div>
                                                </div>
                                                <div class="form-group">
                                                    <label>wage Date</label>
                                                    <input type="date" class="form-control" id="date" placeholder="Enter wage Date" name="date" value="<?php set_value('date', $current_date) ?>">
                                            ...
                                            <script>
                                                document.addEventListener('DOMContentLoaded', function() {
                                                    ...
                                                    // Check if there's a validation error flag
                            ...
                            <div class="panel-body">
                                ...
                                <div class="table-responsive">
                                    <table id="printTable" class="table table-bordered table-hover">
                                        ...
                                        <tbody id="tableBody">
                                            <?php if (!empty($wages)) : $i = 1; ?>
                                                <?php foreach ($wages as $wage) : ?>
                                                    <tr>
                                                        <td><?= $i++ ?></td>
                                                        <td><?php foreach ($employees as $employee) : ?>
                                                                <?php if ($wage['emp_id'] == $employee->emp_id) : ?>
                                                                    <?= $employee->emp_name; ?>
                                                                <?php endif; ?>
                                                            <?php endforeach; ?></td>
                                                        <td><?= $wage['loan_type'] ?></td>
                                                        <td><?= $wage['a_r_type'] ?></td>
                                                        <td><?= $wage['amount'] ?></td>
                                                        <td><?= substr($wage['description'], 0, 10) . '..'; ?></td>
                                                        <td><?= $wage['date'] ?></td>
                                                        <td class="no-print">
                                                            <a href="<?php echo site_url('WagesController/edit_Wages/' . $wage['id']); ?>"><i class="fa fa-pencil" aria-hidden="true" style="color: #007BFF;"></i></a> |
                                                            <a href="<?php echo site_url('WagesController/delete_Wages/' . $wage['id']); ?>"
                                                                onclick="return confirm('Are you sure you want to delete this data?');"><i class="fa fa-trash-o" aria-hidden="true" style="color: red;"></i></a>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else : ?>
